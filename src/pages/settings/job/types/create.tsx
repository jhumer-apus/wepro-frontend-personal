...
export default function CreateJobTypePage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      ...
      setNameError('Job type name is required')
      return
    }

    if (formData.name.length > 100) {
      toast.error('Job type name must not exceed 100 characters')
      setNameError('Job type name must not exceed 100 characters')
      return
    }
  ...
  const handleFormChange = (field: string, value: string | boolean) => {
    ...
    if (field === 'description') {
      const descriptionValue = value as string
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  required
                  maxLength={100}
                  className={`h-12 text-base ${nameError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}`}
                  autoComplete="off"
                />
                <div className="flex justify-between items-center">
                  <p
                    className={`text-xs ${nameError ? 'text-red-500' : 'text-neutral-500 dark:text-neutral-400'}`}
                  >
                    {nameError || ''}
                  </p>
                  <p
                    className={`text-xs ${formData.name.length > 100 ? 'text-red-500' : 'text-neutral-500 dark:text-neutral-400'}`}
                  >
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  maxLength={500}
                  className={`flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${descriptionError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}`}
                  rows={3}
                />
                <div className="flex justify-between items-center">
                  <p
                    className={`text-xs ${descriptionError ? 'text-red-500' : 'text-neutral-500 dark:text-neutral-400'}`}
                  >
                    {descriptionError || ''}
                  </p>
                  <p
                    className={`text-xs ${formData.description.length > 500 ? 'text-red-500' : 'text-neutral-500 dark:text-neutral-400'}`}
                  >
