...
// SMS Character and Segment counting utilities
const getSMSCharacterCount = (text: string): number => {
  if (!text) return 0
  // Remove any HTML tags and count actual characters
  const cleanText = text.replace(/<[^>]*>/g, '')
  return cleanText.length
}

const getSMSSegmentCount = (text: string): number => {
  if (!text) return 0
  const cleanText = text.replace(/<[^>]*>/g, '')
  const charCount = cleanText.length

  // SMS segment calculation:
  // - 160 characters = 1 segment (GSM 7-bit)
  // - 70 characters = 1 segment (Unicode/UTF-16)
  // - For concatenated messages: 153 characters per segment (GSM 7-bit) or 67 characters per segment (Unicode)

  // Check if text contains Unicode characters
  const hasUnicode = /[^\x00-\x7F]/.test(cleanText)
...
export default function SMSTemplatesPage() {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      if (responseData.success) {
        ...
        setError(responseData.message || 'Failed to fetch SMS templates')
        setTemplates([])
        setTotalCount(0)
        setTotalPages(0)
        setPagination({
      ...
      setError(err.response?.data?.message || 'Failed to fetch SMS templates')
      setTemplates([])
      setTotalCount(0)
      setTotalPages(0)
      setPagination({
  ...
  const handlePreviewTemplate = (template: SMSTemplate) => {
    router.push(`/settings/templates/sms/${template._id}/preview`)
  }
  ...
  return (
    <>
      <Head>
        <title>SMS Templates - WePro</title>
        <meta name="description" content="Manage SMS templates" />
      </Head>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button
            onClick={() => router.push('/settings/templates/sms/create')}
            ...
            <CardContent className="pt-6">
              <div className="text-center py-12">
                ...
                <p className="text-neutral-600 dark:text-neutral-400 mb-4">
                  Unable to load SMS templates. Please ensure you are logged in
                ...
                <div className="mt-4 flex flex-wrap gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-neutral-800 dark:text-neutral-200 font-mono">
                      {getSMSCharacterCount(selectedTemplate.template)}
                    </span>
                  </div>
                  {/*<div className="flex items-center gap-2">
                    <span className="text-neutral-600 dark:text-neutral-400 font-medium">SMS Segments:</span>
                    <span className="text-neutral-800 dark:text-neutral-200 font-mono">
                      {getSMSSegmentCount(selectedTemplate.template)}
                    </span>
                  </div>*/}
                </div>
              )}
